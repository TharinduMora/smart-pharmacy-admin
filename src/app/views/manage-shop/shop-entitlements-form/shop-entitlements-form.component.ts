...
@Component({
  selector: 'app-shop-entitlements-form',
  templateUrl: './shop-entitlements-form.component.html',
  styleUrls: ['./shop-entitlements-form.component.scss']
})
export class ShopEntitlementsFormComponent implements OnInit {
  ...
  ngOnInit() {
    setTimeout(() => {
      ...
      this.getShopAssignedEntitle (this.data.shopId);
      this.getShopUnAssignedEntitle (this.data.shopId);
    }, 0);
  }
  ...
  onClickSave () {
    ...
    this.masterMgtSev.assignShopEntitlements(req)
      .then((data: any) => {
        // console.log(data);
  ...
  private getShopAssignedEntitle (shopId: number) {
    this.assignedFunctions = [];
    this.masterSev.getAssignedEntitlementsByShop(shopId).then((response: any) => {
      this.assignedFunctions = this.orderArrayByKey(response || [], 'entitlementId');
    }).catch((error: any) => {});
  }

  private getShopUnAssignedEntitle (shopId: number) {
    this.unassignedFunctions = [];
