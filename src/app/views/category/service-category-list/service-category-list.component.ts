...
export class ServiceCategoryListComponent implements OnInit {
  ...
  private serviceCategoryTypeList = [];
  ...
  private openProductCategoryModel() {
    ...
    this.bsModalRef.content.categoryFormType = 'SERVICE';
    this.bsModalRef.content.shopType = this.shopTypeSelected || {};
    this.bsModalRef.content.shopDomain = this.domainSelected || {};
    this.bsModalRef.content.categoryTypeList = this.serviceCategoryTypeList || [];
  ...
  private initGridConfig() {
    ...
    this.gridConfig = {
      'apiSev': null,
      'sevFunction': 'serviceCategoryFindByCriteria',
      'apiParameters': {
        'statuses' : [
    ...
    this.gridConfig.columns.push({
      ...
      'filterConfig': {
        ...
        'defaultOperator': 'eq',
        'type': 'option',
        'options': this.serviceCategoryTypeList || [],
      },
      'dataDisplayCondition': this.serviceCategoryTypeList || [],
      'dataStyle': {'text-align': 'center', 'font-weight': 600}
    });
    this.gridConfig.columns.push({
      'name': 'Is Parent',
  ...
  private updateStatus (record, StatusId) {
    ...
    this.masterMgtSev.updateServiceCategoryStatus(req).then((response: any) => {
      // console.log(response);
      const data = {
  ...
  private getMasterDataByDomainId(domainId: number) {
    const promise = new Promise((resolve, reject) => {
      return this.masterSev.getMasterData(domainId)
        .then((response: any) => {
          // console.log(response);
          this.serviceCategoryTypeList = response[0].service_category_type || [];
          resolve();
        }).catch((error: any) => {
          reject();
        });
